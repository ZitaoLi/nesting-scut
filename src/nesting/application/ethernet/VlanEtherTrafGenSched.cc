...
void VlanEtherTrafGenSched::sendPacket() {
    ...
    int uniqueID = header.rTag.getUniqueID();
    rTag->setUniqueID(uniqueID);
    if (ID_SeqNum.find(uniqueID) == ID_SeqNum.end()) {
        std::pair<int, int> p {uniqueID, 0};
        ID_SeqNum.insert(p);
    } else {
        int n = ID_SeqNum[uniqueID];
        n += 1;
        ID_SeqNum[uniqueID] = n;
    }
    rTag->setSeqNum(ID_SeqNum[uniqueID]);

    // TODO: print info about Stream
    EV_TRACE << getFullPath() << ": Send TSN packet '" << datapacket->getName()
                    << "(Stream ID: " << uniqueID << ", seqNum:" << ID_SeqNum[uniqueID] << ")' at time " << clock->getTime().inUnit(SIMTIME_US)
                    << endl;
