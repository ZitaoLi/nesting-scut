...
protected:
     ...
     * a scheduled object.
     */
    std::vector<std::tuple<int, T, int, int>> entries;
...
public:
    ...
    /**
     *  Return the flow id of the scheduled object at a given index.
     */
    virtual unsigned int getUniqueID(unsigned int index) const {
        return std::get < 2 > (entries[index]);
    }

    /**
     *  Return the flow phase of the scheduled object at a given index.
     */
    virtual unsigned int getPhase(unsigned int index) const {
     ...
     *                        scheduled entry.
     * @param flow id         The flow of of the schedule entry
     * @param flow phase      The flow phase of the schedule entry
     */
    virtual void addEntry(int length, T scheduledObject, int uniqueID, int phase) {
        totalLength += length;
        entries.push_back(make_tuple(length, scheduledObject, uniqueID, phase));
    }
};
