...
protected:
    ...
    unsigned int requestPacketIndex = 0;
...
protected:
    ...
    virtual cPacket* dequeueWithIndex(unsigned int index);

    virtual void handleRequestPacketEvent(uint64_t maxBits, unsigned int index);
...
public:
    ...
    virtual void requestPacket(uint64_t maxBits, unsigned int index = 0);

    virtual bool isExpressQueue();

    virtual cPacket* getPacketFromQueue(int i);

    virtual unsigned int getPacketNum();
};
