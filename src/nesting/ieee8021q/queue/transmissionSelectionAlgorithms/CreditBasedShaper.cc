...
void CreditBasedShaper::handleMessage(cMessage* msg) {
    if (msg->isSelfMessage()) {
        if (msg == &endSpendingCreditMessage) {
            ...
            // call superclass's method to handle "pktEnqueued", "gateStatesChanged" and "pktRequest" event
            // "handlePacketEnqueuedEvent" and "handleGateStateChangedEvent" method have been overrode by this class
            TSAlgorithm::handleMessage(msg);
        }
    } else {
        Packet* packet = check_and_cast<Packet*>(msg);
        handleSendPacketEvent(packet); // delete credit before send a packet
        send(packet, "out");
    }
}
...
void CreditBasedShaper::handleSendPacketEvent(Packet* packet) {
    ...
    scheduleAt(simTime() + transmissionTime(packet), &endSpendingCreditMessage);  // delete credit after send a packet
    updateState(kSpendCredit); // update state to delete credit
}
...
void CreditBasedShaper::handleEndSpendingCreditEvent() {
    ...
    // if queue is empty and credit is positive
    // then reset credit and update state to stay credit
    if (!isPacketReadyForTransmission() && isCreditPositive()) {
        resetCredit();
        updateState(kIdle);
    // if queue is not empty and transmission-gate is open
    // then update state to add credit
    } else if (isPacketReadyForTransmission() && transmissionGate->isGateOpen()) {
        updateState(kEarnCredit);
        if (credit < 0) {
            scheduleAt(idleTimeToZeroCredit(), &reachedZeroCreditMessage); // reach zero credit
        }
    } else {
        updateState(kIdle);
    }
...
void CreditBasedShaper::handleZeroCreditReachedEvent() {
    ...
    // if credit is positive and queue is not empty
    // then notify transmission-gate enqueued event immediately
