...
// case a new schedule is loaded if available.
    ...
    // TODO print Schedule information
    EV_DEBUG << getFullPath() << ": duration time = " << currentSchedule->getUniqueID(scheduleIndex) << endl;
    EV_DEBUG << getFullPath() << ": unique id = " << currentSchedule->getLength(scheduleIndex) << endl;
    EV_DEBUG << getFullPath() << ": phase = " << currentSchedule->getPhase(scheduleIndex) << endl;
...
// TODO there is bug in this method, do not use this method
std::unique_ptr<Schedule<GateBitvector>> GateController::getCurrentSchedule() {
    return std::move(currentSchedule); // this operation will set unique_prt to nullptr
}
