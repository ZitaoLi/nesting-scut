...
void TransmissionGate::initialize() {
    ...
    lengthAwareQueue = getModuleFromPar<LengthAwareQueue>(
...
void TransmissionGate::handleRequestPacketEvent() {
    ...
    ASSERT(!isEmpty());
    tsAlgorithm->requestPacket(maxTransferableBits()); // request transmission-selection-algorithm to request a packet from the queue above
}
...
void TransmissionGate::setGateState(bool gateOpen, bool release) {
    ...
    tsAlgorithm->gateStateSetted(); // new
}
...
unsigned int TransmissionGate::getPakcetNum() {
    return lengthAwareQueue->getPacketNum();
}
}
//namespace nesting
